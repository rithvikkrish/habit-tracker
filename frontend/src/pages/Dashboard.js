...
function Clock() {
  ...
  return (
    <div style={{ padding: '20px 32px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minWidth: '220px', background: 'rgba(0,0,0,0.2)', position: 'relative', overflow: 'hidden' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at center,rgba(0,255,255,0.06) 0%,transparent 70%)' }} />
      {[{ w: 170, d: 20, c: 'rgba(0,255,255,0.1)' }, { w: 130, d: 12, c: 'rgba(99,102,241,0.08)', rev: true }].map((r, i) => (
      ...
      {/* Cyan clock time — matches screenshot */}
      <div style={{ display: 'flex', alignItems: 'baseline', position: 'relative', zIndex: 1 }}>
        <div style={{ fontSize: '2.4rem', fontWeight: 800, letterSpacing: 2, color: '#00d4ff', textShadow: '0 0 20px rgba(0,212,255,0.5)', fontVariantNumeric: 'tabular-nums' }}>{time}</div>
        <div style={{ fontSize: '1rem', fontWeight: 700, color: '#00d4ff', textShadow: '0 0 15px rgba(0,212,255,0.4)', marginLeft: 7 }}>{ampm}</div>
      </div>
      <div style={{ fontSize: '0.72rem', color: 'rgba(0,212,255,0.55)', marginTop: 7, letterSpacing: 2, textTransform: 'uppercase', position: 'relative', zIndex: 1 }}>{date}</div>
    </div>
  );
}
        ...
        {dailyQuote && (
          <div style={{ background: 'rgba(5,3,16,0.8)', backdropFilter: 'blur(20px)', borderRadius: 24, border: '1px solid rgba(99,102,241,0.2)', marginBottom: 24, display: 'flex', overflow: 'hidden', minHeight: 155, boxShadow: '0 0 40px rgba(99,102,241,0.08)', animation: 'fadeInUp 0.6s ease' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 20, padding: '20px 28px', flex: 1, position: 'relative' }}>
              <div style={{ position: 'absolute', right: 0, top: '15%', bottom: '15%', width: 1, background: 'linear-gradient(to bottom,transparent,rgba(99,102,241,0.35),transparent)' }} />
              <div style={{ position: 'relative', flexShrink: 0 }}>
                ...
                <img src={dailyQuote.image_url} alt={dailyQuote.character} onError={e => e.target.style.display = 'none'}
                  style={{ width: 105, height: 125, borderRadius: 16, objectFit: 'cover', border: '2px solid rgba(99,102,241,0.45)', boxShadow: '0 0 25px rgba(99,102,241,0.3)' }} />
              </div>
              <div style={{ flex: 1 }}>
                {/* Cyan character name — matches screenshot */}
                <p style={{ color: '#00d4ff', fontSize: '0.72rem', textTransform: 'uppercase', letterSpacing: '2.5px', margin: '0 0 8px', fontFamily: EMOJI_FONT, textShadow: '0 0 10px rgba(0,212,255,0.4)' }}>{dailyQuote.character}</p>
