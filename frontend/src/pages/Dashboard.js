...
function CanvasCursor() {
  ...
  useEffect(() => {
    ...
    const TUBES = [
      { color: CYAN,      points: [], width: 3.5 },
      { color: PURP,      points: [], width: 2.8 },
      { color: '#06b6d4', points: [], width: 2.2 },
      { color: '#a855f7', points: [], width: 1.8 },
    ];
    const lags = [0.1, 0.07, 0.05, 0.035];
    const pos  = TUBES.map(() => ({ x: mouse.x, y: mouse.y }));
    const MAX  = 90;
    const drawTube = (tube) => {
      if (tube.points.length < 2) return;
      ctx.save(); ctx.globalAlpha=0.1; ctx.shadowBlur=50; ctx.shadowColor=tube.color;
      ctx.strokeStyle=tube.color; ctx.lineWidth=tube.width*5; ctx.lineCap='round'; ctx.lineJoin='round';
      ctx.beginPath(); ctx.moveTo(tube.points[0].x, tube.points[0].y);
      for (let j=1;j<tube.points.length-1;j++){const mx=(tube.points[j].x+tube.points[j+1].x)/2;const my=(tube.points[j].y+tube.points[j+1].y)/2;ctx.quadraticCurveTo(tube.points[j].x,tube.points[j].y,mx,my);}
      ctx.stroke(); ctx.restore();
      ctx.save(); ctx.globalAlpha=0.25; ctx.shadowBlur=22; ctx.shadowColor=tube.color;
      ctx.strokeStyle=tube.color; ctx.lineWidth=tube.width*2; ctx.lineCap='round'; ctx.lineJoin='round';
      ctx.beginPath(); ctx.moveTo(tube.points[0].x, tube.points[0].y);
      for (let j=1;j<tube.points.length-1;j++){const mx=(tube.points[j].x+tube.points[j+1].x)/2;const my=(tube.points[j].y+tube.points[j+1].y)/2;ctx.quadraticCurveTo(tube.points[j].x,tube.points[j].y,mx,my);}
      ctx.stroke(); ctx.restore();
      for (let j=1;j<tube.points.length;j++){
        const alpha=j/tube.points.length;
        ctx.save(); ctx.globalAlpha=alpha*0.9; ctx.shadowBlur=12; ctx.shadowColor=tube.color;
        ctx.strokeStyle=tube.color; ctx.lineWidth=tube.width*(0.3+alpha*0.7); ctx.lineCap='round';
        ctx.beginPath(); ctx.moveTo(tube.points[j-1].x,tube.points[j-1].y); ctx.lineTo(tube.points[j].x,tube.points[j].y);
        ctx.stroke(); ctx.restore();
      }
    };
    const animate = () => {
      ...
      TUBES.forEach((tube,i)=>{
        ...
        const angle=Math.atan2(mouse.y-pos[i].y,mouse.x-pos[i].x)+Math.PI/2;
        const spread=(i-1.5)*7;
        tube.points.push({x:pos[i].x+Math.cos(angle)*spread,y:pos[i].y+Math.sin(angle)*spread});
        if(tube.points.length>MAX) tube.points.shift();
        drawTube(tube);
      });
      animId=requestAnimationFrame(animate);
    };
    animate();
    const onClick=()=>{const rc=()=>'#'+Math.floor(Math.random()*16777215).toString(16).padStart(6,'0'); TUBES.forEach(t=>t.color=rc());};
    document.body.addEventListener('click',onClick);
    return ()=>{ window.removeEventListener('resize',resize); cancelAnimationFrame(animId); document.body.removeEventListener('click',onClick); };
  },[]);
...
function MatrixBG() {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      drops = Array(Math.floor(cv.width / fontSize)).fill(1);
    };
    const fontSize = 14;
    let drops = [];
    resize();
    ...
    const draw = () => {
      frame++;
      if (frame % 6 !== 0) { animId = requestAnimationFrame(draw); return; }
  ...
  return <canvas ref={canvasRef} style={{ position:'fixed', inset:0, zIndex:0, pointerEvents:'none', opacity:0.13 }} />;
}
...
function Clock() {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setT({ time:`${h}:${String(n.getMinutes()).padStart(2,'0')}:${String(n.getSeconds()).padStart(2,'0')}`, ampm:ap, date:`${days[n.getDay()]}, ${months[n.getMonth()]} ${n.getDate()}` });
    };
...
export default function Dashboard() {
  ...
  const addHabit = async () => {
    ...
    try { await api.post('/api/habits', { name: newHabit, emoji: habitEmoji }); setNewHabit(''); fetchHabits(); } catch {}
  };
  const toggleHabit = async (id) => {
    const today = new Date().toISOString().split('T')[0];
    try { await api.put(`/api/habits/${id}/toggle`, { date: today }); fetchHabits(); } catch {}
  };
  const deleteHabit = async (id) => {
    try { await api.delete(`/api/habits/${id}`); fetchHabits(); } catch {}
  };
  ...
  const getWeeklyTaskData = () => {
    const days=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
    return Array.from({length:7},(_,i)=>{const d=new Date();d.setDate(d.getDate()-(6-i));const ds=d.toISOString().split('T')[0];return{label:days[d.getDay()],value:tasks.filter(t=>t.created_at&&t.created_at.startsWith(ds)).length};});
  };
  ...
  const getWeeklyHabitData = () => {
    const days=['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
    return Array.from({length:7},(_,i)=>{const d=new Date();d.setDate(d.getDate()-(6-i));const ds=d.toISOString().split('T')[0];return{label:days[d.getDay()],value:habits.filter(h=>h.history?.includes(ds)).length};});
  };
  const getMonthlyHabitData = () => {
    const months=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
    return Array.from({length:6},(_,i)=>{const d=new Date();d.setMonth(d.getMonth()-(5-i));const ym=`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}`;return{label:months[d.getMonth()],value:habits.reduce((a,h)=>a+(h.history?.filter(dt=>dt.startsWith(ym)).length||0),0)};});
  };
      ...
      <div style={{position:'relative',zIndex:5,maxWidth:1200,margin:'0 auto',padding:'32px 28px'}}>
        ...
        {activeTab==='habits' && (
          <div style={{animation:'fadeUp 0.3s ease'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:10}}>
              {!habitsLoading&&habits.length===0&&<div style={{...card,textAlign:'center',color:'rgba(255,255,255,0.22)',padding:36}}>No habits yet — add one above!</div>}
              {habits.map(habit=>{
                const today=new Date().toISOString().split('T')[0];
              ...
              {[
                ...
                <div key={s.label} style={{...card,display:'flex',alignItems:'center',gap:14,padding:'16px 20px'}}>
                  ...
                  <div>
                    ...
                    <div style={{fontSize:'0.7rem',color:'rgba(255,255,255,0.3)',letterSpacing:'1px',marginTop:4,fontWeight:600}}>{s.label}</div>
                  </div>
                </div>
              ))}
            </div>
