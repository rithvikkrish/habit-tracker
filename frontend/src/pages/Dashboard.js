...
// ── Apple-style emoji via Twemoji SVG ──────────────────────────
...
function Emoji({ e, size = '1.5rem', style = {} }) {
  return (
    <img src={getEmojiUrl(e)} alt={e}
      style={{ width: size, height: size, verticalAlign: 'middle', display: 'inline-block', ...style }}
      onError={el => { el.target.style.display = 'none'; }}
    />
  );
}

// ── Animated BG (stars + cyberpunk grid) ──────────────────────
function AnimatedBG() {
  ...
  useEffect(() => {
    ...
    const stars = Array.from({ length: 200 }, () => ({
      ...
      r: Math.random() * 1.5 + 0.2, tw: Math.random() * Math.PI * 2, sp: Math.random() * 0.006 + 0.002,
      color: ['#fff', '#c4b5fd', '#93c5fd', '#e0e7ff'][Math.floor(Math.random() * 4)]
    }));
    const nebulas = [
    ...
    const animate = () => {
      frame++;
      ctx.fillStyle = 'rgba(5,3,16,0.18)'; ctx.fillRect(0, 0, cv.width, cv.height);
      nebulas.forEach(n => {
      ...
      stars.forEach(s => {
        ...
        ctx.fillStyle = s.color; ctx.beginPath(); ctx.arc(s.x, s.y, s.r, 0, Math.PI * 2); ctx.fill(); ctx.restore();
      });
      ...
      for (let y = -80 + gridOff % 80; y < cv.height + 80; y += 80) {
        ...
        ctx.strokeStyle = `rgba(99,102,241,${gi * (1 - dc * 0.5)})`; ctx.lineWidth = 1;
        ctx.shadowBlur = 8; ctx.shadowColor = 'rgba(99,102,241,0.4)';
        ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(cv.width, y); ctx.stroke();
      }
      for (let x = 0; x < cv.width + 80; x += 80) {
        const dc = Math.abs(x - cv.width / 2) / (cv.width / 2);
        ctx.strokeStyle = `rgba(139,92,246,${gi * (1 - dc * 0.4)})`; ctx.lineWidth = 1;
        ctx.shadowBlur = 8; ctx.shadowColor = 'rgba(139,92,246,0.4)';
      ...
      if (frame % 300 === 0) {
        ...
        const dss = () => {
          if (ss.op <= 0) return;
          ctx.save(); ctx.globalAlpha = ss.op; ctx.strokeStyle = '#c4b5fd'; ctx.lineWidth = 2;
          ctx.shadowBlur = 15; ctx.shadowColor = '#8b5cf6';
          ctx.beginPath(); ctx.moveTo(ss.x, ss.y); ctx.lineTo(ss.x - Math.cos(ss.a) * ss.len, ss.y - Math.sin(ss.a) * ss.len);
          ctx.stroke(); ctx.restore(); ss.x += ss.spd; ss.y += ss.spd; ss.op -= 0.015; requestAnimationFrame(dss);
        };
        dss();
      }
  ...
  useEffect(() => {
    ...
    sc.textContent = `
      ...
      if(el && !el.dataset.init){
        el.dataset.init='1';
        const app=TubesCursor(el,{tubes:{colors:["#6366f1","#8b5cf6","#06b6d4"],lights:{intensity:160,colors:["#a78bfa","#6366f1","#06b6d4","#8b5cf6"]}}});
        document.body.addEventListener('click',()=>{const rc=()=>"#"+Math.floor(Math.random()*16777215).toString(16).padStart(6,'0');app.tubes.setColors([rc(),rc(),rc()]);app.tubes.setLightsColors([rc(),rc(),rc(),rc()]);});
      }`;
    document.head.appendChild(sc);
    return () => { try { document.head.removeChild(sc); } catch {} };
  }, []);
  return <div id="tubes-canvas" style={{ position: 'fixed', inset: 0, zIndex: 1, pointerEvents: 'none' }} />;
}
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const iv = setInterval(fetchDailyQuote, 60 * 60 * 1000);
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050310', fontFamily: "'Outfit',sans-serif", color: '#fff', position: 'relative' }}>
      ...
      <nav style={{ position: 'relative', zIndex: 10, background: 'rgba(5,3,16,0.88)', backdropFilter: 'blur(20px)', borderBottom: '1px solid rgba(99,102,241,0.15)', padding: '0 28px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', height: '62px' }}>
        <span style={{ fontWeight: 800, fontSize: '1.3rem', background: 'linear-gradient(135deg,#6366f1,#a78bfa)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', display: 'flex', alignItems: 'center', gap: 8 }}>
          <Emoji e="⚡" size="1.4rem" /> TaskMaster
        </span>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <span style={{ color: 'rgba(255,255,255,0.45)', fontSize: '0.875rem' }}>Hey, {user.name || 'User'} <Emoji e="👋" size="1rem" /></span>
      ...
      <div style={{ position: 'relative', zIndex: 5, maxWidth: 1150, margin: '0 auto', padding: '28px 20px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(140px,1fr))', gap: 14, marginBottom: 24 }}>
          {[
            ...
            <div key={stat.label} style={{ ...s.card, textAlign: 'center', transition: 'all 0.3s', cursor: 'default', animation: `fadeInUp ${0.4 + i * 0.1}s ease` }}
              ...
              <div style={{ marginBottom: 10 }}><Emoji e={stat.icon} size="2.2rem" /></div>
        ...
        <div style={{ display: 'flex', gap: 6, marginBottom: 20, background: 'rgba(5,3,16,0.7)', padding: 5, borderRadius: 14, border: '1px solid rgba(99,102,241,0.12)', width: 'fit-content' }}>
          {[{ id: 'tasks', icon: '📋', label: 'Tasks' }, { id: 'habits', icon: '💪', label: 'Habits' }, { id: 'analytics', icon: '📊', label: 'Analytics' }].map(tab => (
            <button key={tab.id} onClick={() => setActiveTab(tab.id)} style={{ padding: '9px 22px', borderRadius: 10, border: 'none', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", fontWeight: 600, fontSize: '0.875rem', transition: 'all 0.3s', background: activeTab === tab.id ? 'linear-gradient(135deg,#6366f1,#8b5cf6)' : 'transparent', color: activeTab === tab.id ? '#fff' : 'rgba(255,255,255,0.35)', boxShadow: activeTab === tab.id ? '0 0 20px rgba(99,102,241,0.4)' : 'none', display: 'flex', alignItems: 'center', gap: 6 }}>
              <Emoji e={tab.icon} size="1rem" /> {tab.label}
            </button>
          ))}
        </div>
        ...
        {activeTab === 'tasks' && (
          <div style={{ animation: 'fadeInUp 0.4s ease' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, flexWrap: 'wrap', gap: 10 }}>
              <h2 style={{ margin: 0, fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: 8 }}><Emoji e="🗒️" size="1.3rem" /> My Tasks</h2>
                ...
                <div key={task.id} style={{ background: 'rgba(5,3,16,0.75)', borderRadius: 14, border: '1px solid rgba(99,102,241,0.12)', padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', transition: 'all 0.3s' }}
                  ...
                  <div style={{ flex: 1, minWidth: 150 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      <span style={{ fontSize: '0.68rem', padding: '3px 10px', borderRadius: 6, background: `${priorityColor[task.priority]}18`, color: priorityColor[task.priority], fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
                        <Emoji e={priorityEmoji[task.priority]} size="0.9rem" /> {task.priority}
                      </span>
                      <span style={{ fontSize: '0.68rem', padding: '3px 10px', borderRadius: 6, background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.35)', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 4 }}>
                        <Emoji e="💼" size="0.9rem" /> {task.category}
                      </span>
                    </div>
                  </div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                    ...
                    <button onClick={() => deleteTask(task.id)} style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, color: '#ef4444', padding: '6px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                      <Emoji e="🗑️" size="1.1rem" />
                    </button>
                  </div>
                </div>
              ))}
        ...
        {activeTab === 'habits' && (
          <div style={{ animation: 'fadeInUp 0.4s ease' }}>
            <h2 style={{ marginBottom: 16, fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: 8 }}><Emoji e="💪" size="1.3rem" /> My Habits</h2>
            <div style={{ ...s.card, marginBottom: 16, display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                {emojiPicker.map(e => (
                  <button key={e} onClick={() => setHabitEmoji(e)} style={{ background: habitEmoji === e ? 'rgba(99,102,241,0.2)' : 'transparent', border: habitEmoji === e ? '1px solid rgba(99,102,241,0.5)' : '1px solid transparent', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', transition: 'all 0.2s', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <Emoji e={e} size="1.5rem" />
                  </button>
                ))}
              </div>
              <div style={{ display: 'flex', gap: 10 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        <span style={{ color: '#f59e0b', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}>
                          <Emoji e="🔥" size="1.1rem" /> {habit.streak || 0}
                        </span>
                        <button onClick={() => toggleHabitToday(habit.id)} style={{ ...s.btn, background: doneToday ? 'rgba(16,185,129,0.2)' : undefined, boxShadow: doneToday ? '0 0 15px rgba(16,185,129,0.3)' : undefined, padding: '8px 14px', display: 'flex', alignItems: 'center', gap: 6 }}>
                          <Emoji e={doneToday ? '✅' : '🎯'} size="1rem" /> {doneToday ? 'Done' : 'Mark Done'}
                        </button>
                        <button onClick={() => deleteHabit(habit.id)} style={{ background: 'rgba(239,68,68,0.12)', border: '1px solid rgba(239,68,68,0.2)', borderRadius: 8, color: '#ef4444', padding: '8px 10px', cursor: 'pointer', display: 'flex', alignItems: 'center' }}>
                          <Emoji e="🗑️" size="1.1rem" />
                        </button>
                      </div>
                    </div>
        ...
        {activeTab === 'analytics' && (
          <div style={{ animation: 'fadeInUp 0.4s ease' }}>
            <h2 style={{ marginBottom: 16, fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: 8 }}><Emoji e="📊" size="1.3rem" /> Analytics</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(240px,1fr))', gap: 16 }}>

              <div style={s.card}>
                <h3 style={{ margin: '0 0 16px', fontSize: '1rem', color: 'rgba(167,139,250,0.8)', display: 'flex', alignItems: 'center', gap: 6 }}><Emoji e="✅" size="1rem" /> Task Completion</h3>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
                  <div style={{ position: 'relative', width: 120, height: 120 }}>
                    <svg viewBox="0 0 120 120" style={{ transform: 'rotate(-90deg)' }}>
                      ...
                      <defs><linearGradient id="purpleGrad" x1="0%" y1="0%" x2="100%" y2="0%">
              ...
              <div style={s.card}>
                <h3 style={{ margin: '0 0 16px', fontSize: '1rem', color: 'rgba(167,139,250,0.8)', display: 'flex', alignItems: 'center', gap: 6 }}><Emoji e="🔥" size="1rem" /> Habit Streaks</h3>
                ...
                {habits.map(h => (
                  <div key={h.id} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                    <Emoji e={h.emoji} size="1.2rem" />
                    <span style={{ flex: 1, fontSize: '0.875rem' }}>{h.name}</span>
                    <span style={{ color: '#f59e0b', fontWeight: 700, display: 'flex', alignItems: 'center', gap: 4 }}><Emoji e="🔥" size="0.9rem" /> {h.streak || 0}</span>
                  </div>
                ))}
              </div>

              <div style={s.card}>
                <h3 style={{ margin: '0 0 16px', fontSize: '1rem', color: 'rgba(167,139,250,0.8)', display: 'flex', alignItems: 'center', gap: 6 }}><Emoji e="📌" size="1rem" /> Tasks by Priority</h3>
                {['high', 'medium', 'low'].map(p => {
                  const count = tasks.filter(t => t.priority === p).length;
                  return (
                    <div key={p} style={{ marginBottom: 12 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5, alignItems: 'center' }}>
                        <span style={{ fontSize: '0.875rem', color: priorityColor[p], display: 'flex', alignItems: 'center', gap: 4 }}>
                          <Emoji e={priorityEmoji[p]} size="0.9rem" /> {p}
                        </span>
